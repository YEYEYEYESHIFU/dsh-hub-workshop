...
const harnessProfileProbe = (_ctx, config = {}) => {
  ...
  const ready = { capability: 'profile-probe', version: '1.0.0', marker: config.marker || 'unset', pid: process.pid }
  writeFileSync(join(root, 'ready.json'), `${JSON.stringify(ready)}\n`, 'utf8')
  return () => {
    writeFileSync(join(root, 'disposed.json'), `${JSON.stringify({ version: '1.0.0', pid: process.pid })}\n`, 'utf8')
  }
}
